...
const SignupForm = () => {
  ...
  const [activeSubStep, setActiveSubStep] = useState(0); // Added this missing state variable
  const [did, setDid] = useState(null); // Moved inside the component
  const [jwt, setJwt] = useState(null); // Moved inside the component
  const [chosenTier, setChosenTier] = useState(null); // Added missing state variable
